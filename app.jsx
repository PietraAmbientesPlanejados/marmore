...
import { PlusCircle, Trash2, Edit2, Save, X, Move, Grid, FileText, Home, Package, Printer } from 'lucide-react';
...
const SistemaOrcamentoMarmore = () => {
  ...
  const gerarPDFComJsPDF = () => {
    ...
    // Criar PDF com páginas do tamanho da etiqueta
    const pdf = new jsPDF({
    ...
    todasPecas.forEach((peca, index) => {
      ...
      const loteNum = String(orcamentoAtual.id).slice(-4);
      
      // HEADER PRETO
      pdf.setFillColor(0, 0, 0);
      pdf.rect(0, 0, 100, 10, 'F');
      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'bold');
      pdf.text(`${orcamentoAtual.id}-Divisoria BR`, 3, 6);
      
      // NOME DA PEÇA
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(11);
      pdf.setFont('helvetica', 'bold');
      pdf.text(peca.nome || 'Peça sem nome', 3, 16);
      
      // DIMENSÕES
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.text(`Comp.: ${peca.comprimento.toFixed(2)}mm`, 3, 24);
      pdf.setFontSize(10);
      pdf.text(`Larg.: ${peca.altura.toFixed(2)}mm`, 3, 30);
      
      // MATERIAL
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'bold');
      pdf.text(material?.nome || 'N/D', 3, 36);
      ...
      // Lote (preto)
      pdf.setFillColor(0, 0, 0);
      pdf.rect(startX, 12, boxW, boxH, 'F');
      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(7);
      pdf.text('Lote:', startX + 2, 16);
      pdf.setFontSize(12);
      pdf.text(loteNum, startX + 2, 21);
      
      // Chapa (branco com borda)
      pdf.setFillColor(255, 255, 255);
      pdf.rect(startX, 24, boxW, boxH, 'FD');
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(7);
      pdf.text('Chapa:', startX + 2, 28);
      pdf.setFontSize(12);
      pdf.text(chapaNum, startX + 2, 33);
      
      // Peça (branco com borda)
      pdf.rect(startX, 36, boxW, boxH, 'FD');
      pdf.setFontSize(7);
      pdf.text('Peça:', startX + 2, 40);
      pdf.setFontSize(12);
      pdf.text(String(index + 1), startX + 2, 45);
      
      // Montagem (preto)
      pdf.setFillColor(0, 0, 0);
      pdf.rect(startX, 48, boxW, boxH, 'F');
      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(7);
      pdf.text('Montagem:', startX + 2, 52);
      pdf.setFontSize(12);
      pdf.text(String(index + 1), startX + 2, 57);
      
      // FOOTER (cinza claro)
      pdf.setFillColor(230, 230, 230);
      pdf.rect(0, 52, 100, 8, 'F');
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(8);
      pdf.setFont('helvetica', 'bold');
      pdf.text(orcamentoAtual.nome, 3, 57);
      pdf.text(String(orcamentoAtual.id), 85, 57);
    });
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-3 gap-3 mt-2">
                            ...
                            <div>
                              ...
                              <p className="text-sm font-semibold text-orange-600">R$ {mat.custo.toFixed(2)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-slate-500">Venda</p>
                              <p className="text-sm font-semibold text-green-600">R$ {(mat.venda || mat.custo).toFixed(2)}</p>
                            </div>
                          </div>
                        </div>
...
const PreviewAcabamentos = ({ peca, mostrarSempre = false, mini = false }) => {
  ...
  const desenharPreview = () => {
    ...
    if (!mini) {
      ...
      ctx.fillRect(offsetX + w/2 - 35, offsetY - 25, 70, 18);
      ctx.strokeStyle = '#64748b';
      ctx.lineWidth = 1;
      ctx.strokeRect(offsetX + w/2 - 35, offsetY - 25, 70, 18);
      ctx.fillStyle = '#1e293b';
      ctx.fillText(`${largura} mm`, offsetX + w/2, offsetY - 13);
      
      // Dimensão vertical
      ctx.save();
      ctx.translate(offsetX - 25, offsetY + h/2);
      ...
      if (peca.nome) {
        ...
        ctx.fillRect(150 - textWidth/2 - 8, 4, textWidth + 16, 20);
        
        ctx.fillStyle = '#ffffff';
        ctx.fillText(nomeExibir, 150, 17);
      }
    }
  };
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center py-3 border-b-2 border-slate-300 mb-3">
          ...
          <div className="flex gap-6">
            <div className="text-right">
              ...
              <span className="font-bold text-lg text-orange-600">R$ {orcamento.custoChapas.toFixed(2)}</span>
            </div>
            <div className="text-right">
              <div className="text-xs text-slate-500 uppercase">Venda</div>
              <span className="font-bold text-lg text-green-600">R$ {orcamento.vendaChapas.toFixed(2)}</span>
            </div>
          </div>
        </div>
          ...
          return (
            <div key={materialId} className="flex justify-between text-sm text-slate-700 pl-4 py-2 hover:bg-slate-50 rounded">
              ...
              <div className="flex gap-6 ml-4">
                <span className="text-orange-600 w-24 text-right">R$ {custoParcial.toFixed(2)}</span>
                <span className="text-green-600 w-24 text-right">R$ {vendaParcial.toFixed(2)}</span>
              </div>
            </div>
          );
        })}
        {orcamento.margemChapas > 0 && (
          <div className="mt-2 pt-2 border-t border-slate-200 flex justify-between text-sm pl-4">
            ...
            <span className="font-semibold text-blue-600">R$ {orcamento.margemChapas.toFixed(2)} ({((orcamento.margemChapas / orcamento.vendaChapas) * 100).toFixed(1)}%)</span>
          </div>
        )}
      </div>
      ...
      <div className="mt-6 space-y-3">
        <div className="flex justify-between py-3 border-t-2 border-slate-400 bg-gradient-to-r from-slate-50 to-slate-100 px-4 rounded-lg">
          ...
          <span className="text-lg font-bold text-orange-600">R$ {orcamento.custoTotal.toFixed(2)}</span>
        </div>
        <div className="flex justify-between py-3 bg-gradient-to-r from-green-50 to-emerald-50 px-4 rounded-lg border-2 border-green-200">
          ...
          <span className="text-xl font-bold text-green-600">R$ {orcamento.vendaTotal.toFixed(2)}</span>
        </div>
        {orcamento.margemTotal > 0 && (
          <div className="flex justify-between py-3 bg-gradient-to-r from-blue-50 to-indigo-50 px-4 rounded-lg border-2 border-blue-200">
            ...
            <span className="text-lg font-bold text-blue-600">
              R$ {orcamento.margemTotal.toFixed(2)}
