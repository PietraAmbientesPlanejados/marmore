...
const SistemaOrcamentoMarmore = () => {
  ...
  const gerarPDFComJsPDF = () => {
    ...
    todasPecas.forEach((peca, index) => {
      ...
      pdf.rect(boxX, 5, boxW, boxH, 'FD');
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(8);
      pdf.setFont('helvetica', 'bold');
      pdf.text('Chapa:', boxX + 2, 10);
      pdf.setFontSize(14);
      pdf.text(chapaNum, boxX + 2, 15);
      
      // ===== MATERIAL (Topo Esquerdo) =====
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.text(material?.nome || 'Material', 3, 8);
      
      // ===== NOME DA PEÇA =====
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'bold');
      pdf.text(peca.nome || 'Peça sem nome', 3, 15);
      
      // ===== DIMENSÕES (sem decimais) =====
      const comp = Math.round(peca.comprimento);
      const larg = Math.round(peca.altura);
      
      pdf.setFontSize(11);
      pdf.setFont('helvetica', 'bold');
      pdf.text(`Comp: ${comp}`, 3, 23);
      pdf.setFontSize(10);
      pdf.text(`Larg: ${larg}`, 3, 29);
      
      // ===== DESENHO DA PEÇA (Proporcional) =====
      // Calcular escala para caber em 60x25mm
      const maxW = 60;
      const maxH = 25;
      const escalaW = maxW / peca.comprimento;
      const escalaH = maxH / peca.altura;
      ...
      // Adicionar cotas no desenho
      pdf.setFontSize(7);
      pdf.setTextColor(100, 100, 100);
      // Cota horizontal (comprimento)
      pdf.text(String(comp), desenhoX + desenhoW/2 - 3, desenhoY - 1);
      // Cota vertical (largura)
      pdf.text(String(larg), desenhoX - 8, desenhoY + desenhoH/2 + 1);
    });
...
root.render(React.createElement(SistemaOrcamentoMarmore));
